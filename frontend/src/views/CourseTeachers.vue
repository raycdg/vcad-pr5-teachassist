  ...
  <div>
    ...
    >
      <v-card>
        ...
        <v-card-text>
          Are you sure you want to remove teacher "{{ removeTarget?.email }}" from this course?
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn
...
async function fetchCourse() {
  try {
    ...
    courseName.value = `${res.data.disciplineName} - ${res.data.groupName} (${res.data.year})`
  } catch {
...
async function fetchTeachers() {
  ...
  error.value = null
  try {
    const res = await axios.get(`/api/courses/${courseId}/teachers`)
...
async function addTeacher() {
  ...
  error.value = null
  try {
    await axios.post(`/api/courses/${courseId}/assign-teacher`, { teacherId: selectedTeacherId.value })
...
async function doRemove() {
  ...
  error.value = null
  try {
    await axios.delete(`/api/courses/${courseId}/teachers/${removeTarget.value.id}`)
